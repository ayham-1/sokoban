...
pub fn info(comptime fmt: []const u8, args: anytype) void {
    ...
    infoAlloc(fba.allocator(), fmt, args) catch |errorr| {
...
pub fn warn(comptime fmt: []const u8, args: anytype) void {
    var buf: [printBufferSize]u8 = undefined;
    var fba = std.heap.FixedBufferAllocator.init(&buf);
    warnAlloc(fba.allocator(), fmt, args) catch |errorr| {
...
pub fn err(comptime fmt: []const u8, args: anytype) void {
    var buf: [printBufferSize]u8 = undefined;
    var fba = std.heap.FixedBufferAllocator.init(&buf);
    errAlloc(fba.allocator(), fmt, args) catch |errorr| {
...
pub fn debug(comptime fmt: []const u8, args: anytype) void {
    if (builtin.mode == .Debug) {
        var buf: [printBufferSize]u8 = undefined;
        var fba = std.heap.FixedBufferAllocator.init(&buf);
        debugAlloc(fba.allocator(), fmt, args) catch |errorr| {
